...
import { type Metrics, type Row } from '../_sortable'
import AnunciosExplorer, { type RealAdRow } from './_explorer'
...
type Ad = Funnel & { ad_id: string; ad_name: string | null; adset_id: string | null; adset_name: string | null; campaign_id: string | null; campaign_name: string | null }
...
export default async function AnunciosPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const anuncioRows: RealAdRow[] = vendasAnuncio.map((a) => {
    ...
    return {
      ...
      vendas: N(a.vendas), itens: N(a.itens), receita: N(a.receita), spend,
  ...
  const conjRows: Row[] = conjuntos.filter((c) => N(c.spend) > 0)
    .map((c) => ({ id: c.adset_id, lead: [c.adset_name, c.campaign_name], status: statusOf.get(c.adset_id), m: conjM(c), campaignId: c.campaign_id || c.adset_id }))
  const adRows: Row[] = ads.filter((a) => N(a.spend) > 0)
    .map((a) => ({ id: a.ad_id, lead: [a.ad_name, a.adset_name], status: statusOf.get(a.ad_id), m: mOf(a), adsetId: a.adset_id ?? undefined, campaignId: a.campaign_id ?? undefined }))
  ...
  return (
    <DashboardShell active="anuncios">
      ...
      <p style={{ fontSize: 13.5, color: 'var(--sub)', marginBottom: 14 }}>Funil por campanha, conjunto e anúncio. <strong>Clique numa campanha ou conjunto</strong> pra filtrar as tabelas abaixo; clique numa coluna pra ordenar.</p>
      ...
      <div className="rounded-xl p-3 mb-4" style={{ fontSize: 12.5, background: 'rgba(245,113,0,.07)', color: 'var(--sub)', lineHeight: 1.55, border: '1px solid rgba(245,113,0,.18)' }}>
        💡 <strong>Compras / ROAS pixel</strong> = o que o pixel do Meta registrou (estimado, atualiza de hora em hora). <strong>Vendas✓ / Receita✓ / CPA real / ROAS real</strong> = vendas reais da Hotmart, atribuídas ao conjunto pelo id rastreado — é a verdade do caixa. O <strong>real</strong> só existe em <strong>campanha e conjunto</strong>; no nível de <strong>anúncio</strong> (pixel) fica "—", porque o rastreio chega só até o conjunto.
      </div>

      <PeriodFilter range={range} from={sp.from} to={sp.to} periodLabel={periodLabel} />

      <AnunciosExplorer campRows={campRows} conjRows={conjRows} adRows={adRows} anuncioRows={anuncioRows} />
    </DashboardShell>
  )
}
