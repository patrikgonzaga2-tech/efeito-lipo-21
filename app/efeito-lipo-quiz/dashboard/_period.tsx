...
export function PeriodFilter({ range, from, to, periodLabel }: { range: string; from?: string; to?: string; periodLabel: string }) {
  ...
  return (
    <div className="rounded-2xl p-4 mb-5" style={{ background: '#fff', border: '1px solid rgba(0,0,0,.07)' }}>
      ...
      <div style={{ fontSize: 12.5, color: 'var(--mute)', marginTop: 10 }}>Mostrando <strong style={{ color: 'var(--ink)' }}>{periodLabel}</strong></div>
    </div>
  )
}
